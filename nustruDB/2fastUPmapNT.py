...
def retrieve_nucleotide_seq(entryID=None, protein_id=None):
    ...
    with Entrez.efetch(db="nuccore", rettype="fasta_cds_na", retmode="text", id=entryID) as handle:
        for seq_record in SeqIO.parse(handle, "fasta"):
            if protein_id in seq_record.description:
...
def main():
    ...
    df = pd.read_csv('/Users/dominiquefastus/Downloads/uniprotkb_taxonomy_id_562_2024_02_19.tsv', sep='\t', header=0, nrows=400, dtype={"Subcellular location [CC]": object,
    ...
    nucleotide_protein_seqs_df['nucleotide_sequence'] = nucleotide_protein_seqs_df.parallel_apply(get_cds, axis=1)
    nucleotide_protein_seqs_df[['nucleotide_id','nucleotide_sequence']] = nucleotide_protein_seqs_df['nucleotide_sequence'].str.split(':',expand=True)
