...
class UniProtFetcher:
    ...
    def submit_id_mapping(self, from_db, to_db, ids):
        ...
        request = requests.post(
            f"{self.API_URL}/idmapping/run", # post the request to the Uniprot API and run the idmapping
    ...
    def get_id_mapping_results_search(self, url):
        ...
        request = self.session.get(url) # get the request from the updated URL
        self.check_response(request)
        ...
        self.print_progress_batches(0, size, total) # print the progress of the batches
        ...
        for i, batch in enumerate(self.get_batch(request, file_format, compressed), 1):
            ...
            self.print_progress_batches(i, size, total) # update the progress of the batches fetched
        return results
...
def main() -> None:
    ...
    fetcher = UniProtFetcher() # create an instance of the UniProtFetcher class
    
    # submit the ID mapping job and get the job ID
    job_id = fetcher.submit_id_mapping(from_db="UniProtKB_AC-ID", to_db="UniProtKB", ids=ids)
    
    # check if the ID mapping results are ready and get the results
    if fetcher.check_id_mapping_results_ready(job_id):
        link = fetcher.get_id_mapping_results_link(job_id)
        # get the ID mapping results from the search URL and store them in a tsv file with the defined features as columns
        results = fetcher.get_id_mapping_results_search(link + "?format=tsv" + "&fields=accession%2Cgene_primary%2Corganism_name%2Ccc_subcellular_location%2Csequence%2Cxref_embl_full%2Cxref_ccds%2Cxref_refseq_full%2Ccc_alternative_products%2Cft_var_seq") 
